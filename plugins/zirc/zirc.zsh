...
#
# To customize the output you get, you can use the following zstyle styles
# the context is ':zirc:<server>:<channel>'. Most of the below, except
# timestamp-format, specify the parameters available to them in zformat. Please
# refer to the zsh manual for additional information about zformat.
# timestamp-format    (formatted using the zsh strftime builtin)
# msg-format          when who msg
# bg-msg-format       when who where msg (logs only use msg-format)
# private-msg-format  when who msg
# nick-notify-format  when who newnick
# join-notify-format  when who where host
# part-notify-format  when who where msg
# quit-notify-format  when who msg
# topic-format        when who where topic
# mode-format         when who mode details
# invite-format       when who where
# kick-format         when who where whom msg
# pong-format         when
# wallops-format      when who msg
# error-format        when error
#
# in zformat, the codes are:
#    when    : T
#    who     : w
#    newnick : n
#    host    : H
...
#    mode    : M
#
# Additionally, zirc can be configured to log its output to a directory tree
# structure in the same way as ii does. To configure this, set the variable:
#     $ZIRC_LOG_HOME
#
...
# usage: _zirc_printf <style> <options...>
# adds an implicit "when:" to the options using zsh prompt expansion of
# timestamps using the zstyle date-format-string
function _zirc_sprintf {
	local format_type="$1"
	shift
	local time_format_string format_string output

	zstyle -s ":zirc:$ZIRC_SERVER:$ZIRC_CURRENT" timestamp-format time_format_string
	local timestamp="$(print -P "$time_format_string")"

	zstyle -s ":zirc:$ZIRC_SERVER:$ZIRC_CURRENT" ${format_type} format_string
	zformat -f output $format_string "T:$timestamp" "$@"
	print $output
}

# used to record a line. pass the context ("where"), the format to use,
# and any relevant zformat-style parameters.
# currently the directory tree looks kind of like ii, but private messages
# will all appear in the same log.
function _zirc_record {
	local format_type="$1"
	local context="$2"
	shift 2

	local out="$(_zirc_sprintf $format_type "$@")"

	if [[ -n "$ZIRC_LOG_HOME" ]]; then
		local logfilepath="${ZIRC_LOG_HOME}/${ZIRC_SERVER}/${context}"
		mkdir -p ${logfilepath:h}
...
function _zirc_command_PRIVMSG {
	...
	_zirc_record msg-format $context "w:$who" "W:$where" "m:$msg"
}
...
function _zirc_command_NICK {
	...
	_zirc_record nick-notify-format server "w:$who" "n:$newnick"
}
...
function _zirc_command_JOIN {
	...
	_zirc_record join-notify-format $where "w:$who" "H:$at" "W:$where"
	_zirc_isme "$pref" && {
...
function _zirc_command_PART {
	...
	_zirc_record part-notify-format $where "w:$who" "W:$where" "m:$msg"
	_zirc_isme "$pref" && {
...
function _zirc_command_QUIT {
	...
	_zirc_record quit-notify-format server "w:$who" "m:$msg"
}
...
function _zirc_command_TOPIC {
	...
	_zirc_record topic-format $where "w:$who" "W:$where" "t:$msg"
}
...
function _zirc_command_MODE {
	...
	_zirc_record mode-format server "w:$who" "M:$mode" "D:${object}${rest}"
}
...
function _zirc_command_INVITE {
	...
	_zirc_record invite-format server "w:$who" "W:$where"
}
...
function _zirc_command_WALLOPS {
	local pref="$1" comm="$2" msg="$3"
	local who="$(_zirc_prefix_nick "$pref")"
	_zirc_record wallops-format server "w:$who" "m:$msg"
}
...
function _zirc_command_ERROR {
	local pref="$1" comm="$2"
	shift 2
	_zirc_record error-format server "E:${(j: :)@}"
}
...
function zirc_msg {
	...
	_zirc_record msg-format $ZIRC_CURRENT "w:$ZIRC_NICK" "W:$ZIRC_CURRENT" "m:${msg}"
